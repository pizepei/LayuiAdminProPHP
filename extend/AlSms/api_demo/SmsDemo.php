...
class SmsDemo
{
     ...
     * @Effect
     * @param  [type] $Data      [数据]
     * @param  [type] $Templtype [模板]
     * @param  [type] $app       [应用]
     * @param  [type] $setOutId  [流水号]
     * @return [type]            [description]
     */
    public static function sendSms($phone,$Data,$Templtype,$app,$setOutId='') {


        $CodeSMS = config('CodeSMS');
        $setTemplateCode = config('CodeSMSsetTemplateCode');
        ...
        $request->setSignName($CodeSMS['setSignName']);

        // 必填，设置模板CODE，应严格按"模板CODE"填写, 请参考: https://dysms.console.aliyun.com/dysms.htm#/develop/template
        $request->setTemplateCode($setTemplateCode[$Templtype]);
        ...
        // 可选，设置流水号
        // $request->setOutId($setOutId);

        // 选填，上行短信扩展码（扩展码字段控制在7位或以下，无特殊需求用户请忽略此字段）
        // $request->setSmsUpExtendCode("1234567");

        // 发起访问请求
        $acsResponse = static::getAcsClient($CodeSMS)->getAcsResponse($request);
...
$data = ['code'=> Mt_str(5,5)];//Mt_str(6,5)
$response = SmsDemo::sendSms('18028750395',$data,1,1,addNumber(4));
// Object ( [Message] => OK [RequestId] => 1249AEB6-0590-4322-8F85-AC8780EF8B4B [BizId] => 658617724475456887^0 [Code] => OK )
