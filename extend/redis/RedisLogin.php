 ...
 * @Last Modified time: 2018-04-02 11:37:24
 */
namespace redis;
...
class RedisLogin  extends RedisModel{
    ...
    /**
     * [set_hash_token redis hash 存储 token]
     * @Effect
    ...
    public function set_key_token($tokenData)
    {
        ...
        $this->redis->multi(\Redis::MULTI);
             ...
             $this->redis->set($this->name.$tokenData['signature'].'_iat',$tokenData['iat']);//设置key=aa value=1 [true]
             $this->redis->expire($this->name.$tokenData['signature'].'_iat',$tokenData['rememberTime']);//设置失效时间[true | false]

             $this->redis->set($this->name.$tokenData['signature'].'_sub',$tokenData['sub']);//设置key=aa value=1 [true]
             $this->redis->expire($this->name.$tokenData['signature'].'_sub',$tokenData['rememberTime']);//设置失效时间[true | false]

             $this->redis->set($this->name.$tokenData['signature'].'_disp',$tokenData['disp']);//设置key=aa value=1 [true]
             $this->redis->expire($this->name.$tokenData['signature'].'_disp',$tokenData['rememberTime']);//设置失效时间[true | false]
            ...
            //设置登录条件数据
             ...
             $this->redis->expire($this->name.$tokenData['signature'].'_lastTime',86400);//设置失效时间[true | false]
             $this->redis->expire($this->name.$tokenData['signature'].'_rememberTime',86400);//设置失效时间[true | false]
    ...
    /**
     * [get_key_count 获取同时在线的设备数量同时删除超过在的数据]
     * @Effect
     * @param  [type] $uid  [用户id]
    ...
    /**
     * [get_key_token 批量获取redis中的数据（redis版本登录）]
     * @Effect
